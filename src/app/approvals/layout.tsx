    ...
    <div className="flex h-screen bg-[#0a0a0a] text-white overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/10 bg-[#111] flex flex-col">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <Link
            href="/approvals"
            className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${pathname === "/approvals" ? "bg-blue-600/10 text-blue-400 border border-blue-500/20" : "text-gray-400 hover:text-white hover:bg-white/5"}`}
          >
            <span>📊</span>{" "}
            <span className="font-medium text-sm">Approvals List</span>
          </Link>
          ...
          {(session?.user as any)?.role !== "HOD" &&
            (session?.user as any)?.role !== "PRINCIPAL" && (
              <Link
                href="/approvals/new"
